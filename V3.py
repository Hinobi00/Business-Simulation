...
class BusinessSimulation:
    def __init__(self, root):
        ...
        self.day = 1
        self.products = 0
        self.save_slots = ["savegame1.bs", "savegame2.bs", "savegame3.bs"]
        self.json_save_slots = ["savegame1.json", "savegame2.json", "savegame3.json"]
    ...
    def save_game_json(self, slot):
        ...
        with open(self.json_save_slots[slot], "w") as file:
            json.dump(save_data, file)
        messagebox.showinfo("Save Game", f"Game saved successfully to JSON slot {slot + 1}.")

    def load_game(self, slot):
        try:
            with open(self.save_slots[slot], "rb") as file:
    ...
    def game_loop(self):
        ...
        tk.Button(self.root, text="Save game as JSON", command=self.show_save_menu_json).pack(pady=5)
        tk.Button(self.root, text="Quit", command=self.root.quit).pack(pady=5)
    ...
    def show_save_menu_json(self):
        self.clear_screen()
        tk.Label(self.root, text="Select a slot to save as JSON:").pack(pady=10)
        for i in range(3):
            tk.Button(self.root, text=f"Save Slot {i + 1}", command=lambda i=i: self.save_game_json(i)).pack(pady=5)
        tk.Button(self.root, text="Back", command=self.game_loop).pack(pady=10)
